...
export default function LandingPage(){return <div className="landingPage">
  ...
  <header className="landingNav"><Link to="/" className="brand publicBrand"><span className="brandMark">J</span><div><b>Job Radar</b><small>CAREER INTELLIGENCE</small></div></Link><nav aria-label="Primary"><a href="#how">How it works</a><a href="#sources">Sources</a><Link to="/accessibility">Accessibility</Link></nav><div className="landingActions"><Link className="btn secondary" to="/auth?mode=login">Sign in</Link><Link className="btn primary" to="/auth?mode=signup">Create account <ArrowRight size={15}/></Link></div></header>
  ...
  <section id="how" className="landingSection"><p className="eyebrow">HOW IT WORKS</p><div className="featureGrid">{features.map(([title,body,Icon])=><article className="featureCard" key={String(title)}><Icon size={21}/><h2>{String(title)}</h2><p>{String(body)}</p></article>)}</div></section>
  <section id="sources" className="landingSection sourceSection"><div><p className="eyebrow">SOURCE COVERAGE</p><h2>Recent jobs, with the original destination preserved.</h2><p>Automated feeds currently include Remote OK and Arbeitnow, whose public APIs support aggregation. Arbeitnow itself aggregates employer ATS sources such as Greenhouse and SmartRecruiters. LinkedIn and Instagram are only added through approved platform/API access or verified public application links; Job Radar does not bypass platform access controls.</p></div><div className="sourceList"><span>Remote OK</span><span>Arbeitnow</span><span>Employer ATS feeds</span><span>LinkedIn — approved adapter</span><span>Instagram — approved adapter</span></div></section>
  </main>

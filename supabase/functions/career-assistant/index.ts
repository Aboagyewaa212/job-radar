...
Deno.serve(async(req:Request)=>{
  ...
  const url=Deno.env.get('SUPABASE_URL')!,publishable=JSON.parse(Deno.env.get('SUPABASE_PUBLISHABLE_KEYS')??'{}').default
  if(!publishable)return json({error:'Supabase publishable key unavailable'},500)
  ...
  const mode:Mode=body?.mode==='cover_letter'?'cover_letter':'resume';const jobId=String(body?.jobId??'');const sourceText=String(body?.sourceText??'').trim();const userNotes=String(body?.instructions??'').trim().slice(0,2000)
  ...
  if(openaiKey){const instructions=`You are a truthful career application assistant. Never invent employers, dates, degrees, certifications, metrics, tools, responsibilities or achievements. Use only facts supplied in SOURCE CV and JOB DATA. ${mode==='resume'?'Produce a concise ATS-friendly tailored resume in plain text, reordering and rephrasing only when factually supported.':'Produce a concise personalized cover letter in plain text, using only supported evidence.'} Do not add fake claims. If useful evidence is missing, omit it. User notes are preferences, not facts.`;const input=`JOB DATA\nTitle: ${job.title}\nCompany: ${job.company}\nSkills: ${(job.skills??[]).join(', ')}\nRequirements: ${JSON.stringify(job.requirements??[])}\nDescription: ${String(job.description??'').slice(0,12000)}\n\nSOURCE CV\n${source.slice(0,30000)}\n\nUSER NOTES\n${userNotes||'None'}`;try{const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${openaiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5.6-luna',store:false,instructions,input})});if(r.ok){content=extractResponseText(await r.json()).trim();if(content)generationMode='openai'}else console.error('OpenAI response',r.status,await r.text())}catch(error){console.error('OpenAI request failed',error)}}

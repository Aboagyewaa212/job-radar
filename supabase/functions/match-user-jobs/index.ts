...
Deno.serve(async (req: Request) => {
  ...
  const resumeText = (resumeResult.data?.extracted_text ?? '').toLowerCase()
  const evidence = new Set([
    ...normalize(profile?.skills),
    ...normalize(preferences?.target_roles),
    ...normalize(profile?.target_fields),
  ])
  for (const tokenPart of resumeText.split(/[^a-z0-9+#.]+/)) if (tokenPart.length >= 3) evidence.add(tokenPart)

  const { data: jobs, error: jobsError } = await admin
    .from('jobs')
    .select('id,title,company,location,remote_scope,fully_remote,description,requirements,skills,status')
    .eq('status', 'active')
    .limit(500)
  if (jobsError) return json({ error: jobsError.message }, 500)

  const excluded = normalize(preferences?.excluded_keywords)
  const rows = (jobs ?? []).map((job: any) => {
    const jobSkills = normalize(job.skills)
    const text = `${job.title ?? ''} ${job.company ?? ''} ${job.description ?? ''} ${JSON.stringify(job.requirements ?? [])}`.toLowerCase()
    ...
    const matched = [...evidence].filter(skill => skill.length > 2 && (jobSkills.includes(skill) || text.includes(skill))).slice(0, 12)
    ...
    const roleBonus = normalize(preferences?.target_roles).some(role => (job.title ?? '').toLowerCase().includes(role)) ? 15 : 0
    const fit = Math.max(0, Math.min(100, skillScore + remoteBonus + roleBonus))
    return {
      user_id: user.id,
      job_id: job.id,
      fit_score: fit,
      why_match: matched.slice(0, 5),
  ...
  if (rows.length) {
    const { error } = await admin.from('user_job_matches').insert(rows)
